...
const CheckOut = ({ products }) => {
  ...
  const showCheckOut = () => {
    return isAuthenticated() ? (
      ...
      <Link to="/signin">
        <button className="btn-small  red lighten-3">
          Sign in to Checkout
        </button>
      </Link>
    );
  ...
  const showLoading = loading => (
    <h4
      className="center-align black-text text-darken-1"
  ...
  const showError = error => (
    <h6
      className="center-align red-text text-darken-1"
  ...
  const showSuccess = success => (
    <h6
      className="center-align green-text text-darken-1"
